...
static bool is_gen_3_valid(uint8_t scan, uint16_t cols, uint16_t refresh, uint8_t bits, float *clk_mhz, float *gclk_mhz) {
            ...
            for (uint8_t scan = scan_low; scan <= scan_high; scan *= 2) {
                for (uint8_t bits = min_bpp_bits; bits <= max_grayscale_bits; bits++) {
                    ...
                    if ((scan * 2 > cols) && !showAll)
                        continue;
                    else if (is_gen_3_valid(scan, cols, refresh, bits, &clk, &gclk))
