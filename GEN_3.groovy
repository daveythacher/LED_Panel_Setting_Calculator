...
class GEN_3 extends Panel_Calculator {
    ...
    private boolean is_gclk_valid(double uS, short scan, int cols, int refresh, short bits, double dimming) {
        // Calculate number of LED drivers in series.
        double temp = cols / columns_per_driver;
        ...
        // Verify GCLK
        temp = 1000000.0 / refresh;
        temp /= 1 << bits;
        temp /= scan;
        temp *= dimming;

        // True, if a valid driver frequency was found
        return hz_limit >= 1.0 && temp > uS;
    }

    public short frames_per_second = 60;
}